...
class FirebaseService {
  ...
  final CollectionReference _interviewsCollection = FirebaseFirestore.instance.collection('interviews');

  Stream<QuerySnapshot> getInterviews() {
    return _interviewsCollection.snapshots();
  }

  Future<void> addInterview(String name, String date, String location, String status) async {
...
class _InterviewListWidgetState extends State<InterviewListWidget> {
  ...
  void _updateFormCompleted() {
    setState(() {
      _formCompleted = _nameController.text.isNotEmpty && _dateController.text.isNotEmpty && _locationController.text.isNotEmpty;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    child: Column(
    children: <Widget>[
      Container(width: 15, margin: const EdgeInsets.only(left: 200.0, right: 500.0)),
    TextFormField(
    ...
    decoration: const InputDecoration(labelText: 'Location'),
      ...
      const Divider(thickness: 0.50, color: Colors.black,),
      const Text(
        "Upcoming interviews",
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () {
                      widget.firebaseService.deleteInterview(_interviews[index].id);
                    },
                  ),
                );
              },
